...
function CarDetail({ cars }) {
  ...
  useEffect(() => {
    axios.get("http://3.145.83.67:8080/api/loggn", { withCredentials: true })
      .then(response => {
  ...
  const handleLogout = () => {
    axios.get("http://3.145.83.67:8080/api/loggout", { withCredentials: true})
      .then(response => {
        if (response.data.status === "success") {
          setIsLoggedIn(false);
        } else {
  ...
  const { data: favorites, isLoading, refetch } = useQuery({
    queryKey: ['favorites'],
    queryFn: async () => {
      const response = await axios.get("http://3.145.83.67:8080/api/favorites", {
  ...
  const addFavoriteMutation = useMutation({
    mutationFn: async () => {
      const response = await axios.post("http://3.145.83.67:8080/api/favorites",
        {vin: car.VIN},
        {withCredentials: true}
      );
  ...
  const removeFavoriteMutation = useMutation({
    mutationFn: async () => {
      const response = await axios.delete("http://3.145.83.67:8080/api/favorites", {
        data: {vin: car.VIN},
        withCredentials: true
      });
      return response.data;
